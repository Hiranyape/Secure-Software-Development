...
import { onRemoveFromWishlist, onViewProfile, onAddToCart, onRemoveFromCart, onCreateAddress, onPlaceOrder  } from '../store/actions'
...
const Profile = () => {
    ...
    const { user, wishlist, cart, orders, address } = useSelector(state => state.userReducer);
    ...
    const viewCart = () => {
        ...
        if(Array.isArray(cart) && cart.length){
            return <div>
                    {cart.map(item => {
                        if(item){
                            return  <CartItem cart={cart} item={item} onAdd={onAdd} onRemove={onRemove} />
                        }
                    })}
            </div>
        }else{
            return <div className="d-flex justify-content-center align-items-center" style={{ height: '20rem'}}>
                ...
                <div className="col-12">
                    <nav>
                        <div className="nav nav-tabs" id="nav-tab" role="tablist">
                            <a className="nav-link active" style={{ color: '#3680B4'}} id="nav-cart-tab" data-toggle="tab" href="#nav-cart" role="tab" aria-controls="nav-cart" aria-selected="true"><i className="fas fa-shopping-cart mr-3"></i>Cart</a>
                            <a className="nav-link" style={{ color: '#3680B4'}} id="nav-wishlist-tab" data-toggle="tab" href="#nav-wishlist" role="tab" aria-controls="nav-wishlist" aria-selected="false"><i className="fas fa-heart mr-3"></i>Wishlist</a>
                            <a className="nav-link" style={{ color: '#3680B4'}} id="nav-orders-tab" data-toggle="tab" href="#nav-orders" role="tab" aria-controls="nav-orders" aria-selected="false"><i className="fas fa-list-alt mr-3"></i>Orders</a>
                        </div>
                    </nav>

                </div>
            </div>
            <div className="row bg-white" style={{ minHeight: '40rem', padding: 20}}>
                <div className="tab-content container-fluid" id="nav-tabContent" >
                        <div className="tab-pane fade show active" id="nav-cart" role="tabpanel" aria-labelledby="nav-cart-tab">
                            {cart && viewCart() }
 
                        </div>
                        <div className="tab-pane fade" id="nav-wishlist" role="tabpanel" aria-labelledby="nav-wishlist-tab">
